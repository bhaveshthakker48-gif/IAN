 ...
 * @author harism
 */
public class CurlRenderer1 implements GLSurfaceView.Renderer {
	...
	private Vector<CurlMesh1> mCurlMeshes;
	 ...
	 * Basic constructor.
	 */
	public CurlRenderer1(Observer observer) {
		mObserver = observer;
		mCurlMeshes = new Vector<CurlMesh1>();
	 ...
	 */
	public synchronized void addCurlMesh(CurlMesh1 mesh) {
		removeCurlMesh(mesh);
	 ...
	 */
	public synchronized void removeCurlMesh(CurlMesh1 mesh) {
	...
	/*public synchronized void setMargins(float left, float top, float right,
		...
		mMargins.left = left * 0.5f;  // Example: Reduce left margin by half
		mMargins.top = top * 0.5f;    // Reduce top margin by half
		mMargins.right = right * 0.5f;  // Reduce right margin by half
		mMargins.bottom = bottom * 0.5f;  // Reduce bottom margin by half
		updatePageRects();
	}*/
