...
public class CurlView extends GLSurfaceView implements View.OnTouchListener,
	...
	// show page flipping.
	private CurlMesh1 mPageCurl;

	private CurlMesh1 mPageLeft;
	private PageProvider mPageProvider;
	private CurlMesh1 mPageRight;

	private PointerPosition mPointerPos = new PointerPosition();

	private CurlRenderer1 mRenderer;
	...
	private void init(Context ctx) {
		mRenderer = new CurlRenderer1(this);
		...
		mPageLeft = new CurlMesh1(10);
		mPageRight = new CurlMesh1(10);
		mPageCurl = new CurlMesh1(10);
	...
	public void onDrawFrame() {
		...
		if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
			if (mAnimationTargetEvent == SET_CURL_TO_RIGHT) {
				...
				CurlMesh1 right = mPageCurl;
				CurlMesh1 curl = mPageRight;
				...
				CurlMesh1 left = mPageCurl;
				CurlMesh1 curl = mPageLeft;
	...
	public boolean onTouch(View view, MotionEvent me) {
		...
		RectF rightRect = mRenderer.getPageRect(CurlRenderer1.PAGE_RIGHT);
		RectF leftRect = mRenderer.getPageRect(CurlRenderer1.PAGE_LEFT);
			...
			if (mCurlState == CURL_LEFT || mCurlState == CURL_RIGHT) {
				...
				if ((mViewMode == SHOW_ONE_PAGE && mPointerPos.mPos.x > (rightRect.left + rightRect.right) / 2)
					...
					mAnimationTarget.x = mRenderer
							.getPageRect(CurlRenderer1.PAGE_RIGHT).right;
	...
	private void setCurlPos(PointF curlPos, PointF curlDir, double radius) {
		...
		if (mCurlState == CURL_RIGHT
			...
			RectF pageRect = mRenderer.getPageRect(CurlRenderer1.PAGE_RIGHT);
			...
			RectF pageRect = mRenderer.getPageRect(CurlRenderer1.PAGE_LEFT);
			if (curlPos.x <= pageRect.left) {
				mPageCurl.reset();
				requestRender();
				return;
	...
	public void setViewMode(int viewMode) {
		...
		case SHOW_ONE_PAGE:
			mViewMode = viewMode;
			mPageLeft.setFlipTexture(true);
			mRenderer.setViewMode(CurlRenderer1.SHOW_ONE_PAGE);
			break;
		case SHOW_TWO_PAGES:
			mViewMode = viewMode;
			mPageLeft.setFlipTexture(false);
			mRenderer.setViewMode(CurlRenderer1.SHOW_TWO_PAGES);
			break;
		}
	}
	...
	private void startCurl(int page) {
		...
		case CURL_RIGHT: {
			...
			if (mCurrentIndex > 0) {
				mPageLeft.setFlipTexture(true);
				mPageLeft
						.setRect(mRenderer.getPageRect(CurlRenderer1.PAGE_LEFT));
				mPageLeft.reset();
				if (mRenderLeftPage) {
			...
			if (mCurrentIndex < mPageProvider.getPageCount() - 1) {
				...
				mPageRight.setRect(mRenderer
						.getPageRect(CurlRenderer1.PAGE_RIGHT));
				mPageRight.setFlipTexture(false);
				mPageRight.reset();
			...
			mPageCurl.setRect(mRenderer.getPageRect(CurlRenderer1.PAGE_RIGHT));
	...
	private void updateCurlPos(PointerPosition pointerPos) {
		...
		if (mCurlState == CURL_RIGHT
			...
			float pageWidth = mRenderer.getPageRect(CurlRenderer1.PAGE_RIGHT)
					.width();
			...
			if (dist >= curlLen) {
				...
				if (mViewMode == SHOW_TWO_PAGES) {
					...
					float pageLeftX = mRenderer
							.getPageRect(CurlRenderer1.PAGE_RIGHT).left;
		...
		// Otherwise we'll let curl follow pointer position.
			...
			float pageLeftX = mRenderer.getPageRect(CurlRenderer1.PAGE_RIGHT).left;
			radius = Math.max(Math.min(mCurlPos.x - pageLeftX, radius), 0f);

			float pageRightX = mRenderer.getPageRect(CurlRenderer1.PAGE_RIGHT).right;
