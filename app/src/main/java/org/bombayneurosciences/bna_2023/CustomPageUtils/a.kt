 ...
 * @author harism
 */
open class a : GLSurfaceView, OnTouchListener, CurlRenderer1.Observer {
        ...
        set(index) {
            mCurrentIndex = if (mPageProvider == null || index < 0) {
                ...
                if (mAllowLastPageCurl) {
                    min(index.toDouble(), mPageProvider!!.pageCount.toDouble()).toInt()
                } else {
                    min(
                        index.toDouble(),
                        (mPageProvider!!.pageCount - 1).toDouble()
                    ).toInt()
                }
            }
            updatePages()
            ...
            MotionEvent.ACTION_DOWN -> {
                run {
                    ...
                    if (mViewMode == SHOW_TWO_PAGES) {
                        ...
                        val halfX =
                            (rightRect.right + rightRect.left) / 2
    ...
    private fun updateCurlPos(pointerPos: PointerPosition) {
        ...
        radius *= max((1f - pointerPos.mPressure).toDouble(), 0.0)
        ...
        if (mCurlState == CURL_RIGHT
            ...
            val pageWidth = mRenderer!!.getPageRect(CurlRenderer1.PAGE_RIGHT)
                .width()
            var curlLen = radius * Math.PI
            if (dist > (pageWidth * 2) - curlLen) {
                curlLen = max(((pageWidth * 2) - dist).toDouble(), 0.0)
            ...
            val pageLeftX = mRenderer!!.getPageRect(CurlRenderer1.PAGE_RIGHT).left
            radius = max(min((mCurlPos.x - pageLeftX).toDouble(), radius), 0.0)

            val pageRightX = mRenderer!!.getPageRect(CurlRenderer1.PAGE_RIGHT).right
            //(mCurlPos.x -= min((pageRightX - mCurlPos.x).toDouble(), radius))
            mCurlDir.x = mCurlPos.x + mDragStartPos.x
            mCurlDir.y = mCurlPos.y - mDragStartPos.y
        }
